...
- `cmd/api/main.go` wires the database, repository, service, and handlers. Its extracted `newRouter` function owns the Go 1.22 method-aware routes for `GET /health`, `GET /tasks`, `GET /tasks/{id}`, `POST /tasks`, `PUT /tasks/{id}`, and `DELETE /tasks/{id}`. The API serves on `:8080` through an `http.Server` with read-header, read, write, and idle timeouts.
...
- `go test -race -cover ./...` passes with 86.9% overall statement coverage: `cmd/api` 29.6%, `internal/handler` 98.8%, `internal/repository` 88.4%, and `internal/service` 100.0%. `go vet ./...` also passes.
- Resume with phase 9 in `specs/go-task-api-sqlite-guide.html`: add request logging and graceful SIGINT/SIGTERM shutdown. Final polish still includes environment-configurable listen/database paths and ignoring generated SQLite database files.
